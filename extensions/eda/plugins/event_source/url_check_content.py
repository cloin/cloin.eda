...
events with their status and content.
...
async def main(queue: asyncio.Queue, args: dict[str, Any]) -> None:
    """Poll a set of URLs and send events with status and content."""
    urls = args.get("urls", [])
            ...
            async with aiohttp.ClientSession() as session:
                for url in urls:
                    async with session.get(url, verify_ssl=verify_ssl) as resp:
                        content = await resp.json()  # Read the response content
                        await queue.put(
                            {
                                "url_check": {
                                    "url": url,
            ...
            await queue.put(
                {
                    "url_check": {
                        ...
                        "content": "",  # No content in case of a client error
                    },
                },
            )
...
if __name__ == "__main__":
    ...
    asyncio.run(main(MockQueue(), {"urls": ["http://host.containers.local:8080/status.json"]}))
